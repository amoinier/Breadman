...
// Include des différentes librairies utilisées.
#include <stdlib.h>
#include <stdio.h>
#include <SDL/SDL.h>
#include <iostream>
...
{
    Personnage bread, ennemi; // Création des différents objets personnages.

    SDL_Surface *ecran = NULL, *mur = NULL, *arbre = NULL, *textefin = NULL, *textefin2 = NULL, *textefin3 = NULL, *coeur = NULL; // Création de différents poiteurs pour les surfaces.
    SDL_Event event;
    ...
    int carte[NB_BLOCS_LARGEUR][NB_BLOCS_HAUTEUR]; // Tableau qui stock les différents objets de la cartes.
    ...
    SDL_Init(SDL_INIT_VIDEO); // Initialisation de la partie vidéo de la SDL.
    ...
    ecran = SDL_SetVideoMode(LARGEUR_FENETRE, HAUTEUR_FENETRE, 32, SDL_HWSURFACE|SDL_DOUBLEBUF|SDL_FULLSCREEN); // Définition de l'écran.
    SDL_WM_SetCaption( "BreadMan", NULL); // Parametre le nom de la fenêtre.

    // Assignation des image aux différents tableaux pour les sprites.

    // Gestion de l'image du héro.
    ...
    int n = 1; // Variable utilisé pour l'animation du héro.
    ...
    int v = 1; // Variable pour l'animation des décors.
    ...
    // Gestion d'image de differents éléments.
    ...
    exit(EXIT_FAILURE); // Arrêt du programme si 'niveaux.map' n'a pas pu être ouvert.


    // Gestion des positions à l'aide de SDl_Rect.
    // Position hero :
    positionHero.x = LARGEUR_FENETRE/2; // On positionne le héro au milieu de la fenêtre.
    positionHero.y = HAUTEUR_FENETRE/2;
    positionHero.w = 16; // On indique la taille du sprite en pixel.
    positionHero.h = 27;

    // Position de l'ennemie.
    positionEnnemis.x = 5*34; // On positionne le héro au milieu de la fenêtre.
    ...
    // Placement des coeurs au début
    positionCoeur.w = 34;
    positionCoeur.h = 34;
    positionCoeur.x = 1500;
    positionCoeur.y = 1500;

    // Différentes jauges de vie.
    SDL_Surface *jaugeVieImage= SDL_LoadBMP ("Ressources/images/jauge.bmp");//chargement de l'image de la jauge

    // Jauge de l'ennemie.
    SDL_Rect jaugeDeVieEnnemiPosition;//création d'un sdl rect pour la position de la jauge
    SDL_Rect jaugeDeVieEnnemiValeur;//création d'un sdl rect pour la selection de la partie de l'image de la jauge voulue
    ...
    // Active la répétition des touches pour éviter d'appuyer plusieurs fois sur les touches
    SDL_EnableKeyRepeat(1,1);

    // Parametres des armes et de la défense des personnages.
    bread.changerArme(1);
    ...
    while(vraie) // Boucle principale.
    {
        ...
        va de 0 à 100 on multiplie par 0.16).*/

        // Actualisation de la jauge du héroe en fonction de sa vie et de sa position.
        jaugeDeVieHeroePosition.x = positionHero.x;
        jaugeDeVieHeroePosition.y = positionHero.y-5;
        ...
        tempsActuel = SDL_GetTicks(); // Affectation à tempsActuel, le temps en secondes.
        ...
        tempsPrecedent2 = tempsActuel; // On reaffecte à temps précedent le temps actuel.
        if (v>3){v=0;} // Si on dépasse 4 on retourne à 1.
        nouvelleFeuilleFrame = feuilleFrame[0][v]; // On stock l'image de la feuille dans la variable finale.
        v++;
        }

        /* Ces lignes de codes permettent de géré les déplacement et les collisions avec un ennemi,
        ...
        if (keystates[SDLK_UP]){
            variableSensHeroe = 0;
            // Tout le code jusqu'au 'deplacement' sert à animer le sprite avec 4 images.
            if (tempsActuel - tempsPrecedent > 100){ // Si 100ms ce sont écoulé.
                tempsPrecedent = tempsActuel; // On affecte a la variable temps précédent le temps actuel.
                if (n>3){n=0;} // Si n est supérieur à 4 on retourne à 1 pour faire l'animation en boucle.
                    nouveauSensHeroe = heroeSens[0][n]; // On affecte l'image à charger.
                    n++; // On incrémente n.
                }
        ...
        if ((((positionHero.x/TAILLE_BLOC) == positionEnnemis.x/TAILLE_BLOC) && ((positionHero.y/TAILLE_BLOC)-1 == positionEnnemis.y/TAILLE_BLOC)) ||
            ((((positionHero.x/TAILLE_BLOC) == positionEnnemis.x/TAILLE_BLOC) && (positionHero.y/TAILLE_BLOC)-2 == positionEnnemis.y/TAILLE_BLOC))){
                if(collision(&positionEnnemis, &positionHero, 2) == 0){
                    if (tempsActuel - tempsPrecedent4 > 100){ // Si 100ms se sont écoulé.
                        tempsPrecedent4 = tempsActuel; // On affecte a la variable temps précédent le temps actuel.
                        if (w>3){w=0;} // Si w est supérieur à 4 on retourne à 1 pour faire l'animation en boucle.
                            nouveauSensEnnemi = ennemiSens[1][w]; // On affecte l'image à charger.
                            w++; // On incrémente w.
                        }
                ...
                if(collision(&positionEnnemis, &positionHero, 1) == 0){
                    if (tempsActuel - tempsPrecedent4 > 100){ // Si 100ms se sont écoulé.
                        tempsPrecedent4 = tempsActuel; // On affecte a la variable temps précédent le temps actuel.
                        if (w>3){w=0;} // Si w est supérieur à 4 on retourne à 1 pour faire l'animation en boucle.
                            nouveauSensEnnemi = ennemiSens[0][w]; // On affecte l'image à charger.
                            w++; // On incrémente w.
                        }
                ...
                if(collision(&positionEnnemis,&positionHero, 4) == 0){
                    if (tempsActuel - tempsPrecedent4 > 100){ // Si 100ms ce sont écoulé.
                        tempsPrecedent4 = tempsActuel; // On affecte a la variable temps précédent le temps actuel.
                        if (w>3){w=0;} // Si w est supérieur à 4 on retourne à 1 pour faire l'animation en boucle.
                            nouveauSensEnnemi = ennemiSens[3][w]; // On affecte l'image à charger.
                            w++; // On incrémente w.
                        }
                ...
                if(collision(&positionEnnemis,&positionHero, 3) == 0){
                    if (tempsActuel - tempsPrecedent4 > 100){ // Si 100ms ce sont écoulé.
                        tempsPrecedent4 = tempsActuel; // On affecte a la variable temps précédent le temps actuel.
                        if (w>3){w=0;} // Si w est supérieur à 4 on retourne à 1 pour faire l'animation en boucle.
                            nouveauSensEnnemi = ennemiSens[2][w]; // On affecte l'image à charger.
                            w++; // On incrémente w.
                        }
            ...
            if (tempsActuel - tempsPrecedent5 > aleatoire()){
                ...
                if(collision(&positionEnnemis,&positionHero, 3) == 1 || collision(&positionEnnemis,&positionHero, 1) == 1 || collision(&positionEnnemis,&positionHero, 2) == 1 || collision(&positionEnnemis,&positionHero, 4) == 1){
                        for (em=0; em<4; em++){ // On fais une boucle.
                            switch (variableSensEnnemi){ // En fonction du sens de l'ennemi, on positione l'image différement.
                                case 0 :
                        ...
                        nouveauSensArmeEnnemi = armeSensEnnemie[variableSensEnnemi][em]; // On déplace le rectange d'afffichage dans l'image spriteVent.
                    ...
                    case SDLK_SPACE:
                        ...
                        if (tempsActuel - tempsPrecedent > 300){ // On met un timer pour ralentir le temps entre 2 mouvements.
                            ...
                            for (im=0; im<4; im++){ // On fais une boucle.
                                switch (variableSensHeroe){ // On fonction du sens du heroe on position l'image différement.
                                    case 0 :
                                ...
                                nouveauSensArme = armeSens[variableSensHeroe][im]; // On déplace le rectange d'afffichage dans l'image spriteVent.
                                ...
                                SDL_BlitSurface(imageArmeE, &nouveauSensArmeEnnemi, ecran, &positionArmeE);
                                SDL_Flip(ecran); // On rafraîchit l'écran.
                                SDL_Delay(15); // On fais une pause de 15ms.
                                }
                            im = 0;
                            positionArme.x = 1500; // On efface l'arme en la mettant en dehors de l'écran.
                            positionArme.y = 1500;
                            }
        ...
        for (i = 0 ; i < NB_BLOCS_LARGEUR ; i++) // Boucle pour charger les différente image en fonction du tableau 'carte'.
        {
